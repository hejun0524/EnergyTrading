...
function _construct_PPO_memory(n_states::Int, n_actions::Int)::PPOMemory
    return PPOMemory(n_states = n_states, n_actions = n_actions)
end
...
function _modify_memory!(
    ...
    if reward !== nothing
        _accumulate_raw_reward!(buffer.reward_memory[end], reward, replace = reward_replace)
    end
    if done !== nothing
...
function _clear_buffer!(buffer::PPOMemory)
    buffer.state_memory = []
